...
{
    return _mm256_mul_ps(a, b); // Ejercicio A
}
...
static inline float simd_reduce_add_ps(__m256 value)
{
    ...
    return _mm_cvtss_f32(sum); // Ejercicio B
}

float dot_product_avx2(const float a[VECTOR_SIZE], const float b[VECTOR_SIZE])
{
    float result = 0.0f;
//For loop que brinque de avx_floars elementos
// cargar el vector usando simd_loadu_ps
//multiplicar simd_mul_ps
// multiplucar simd_reduce add_ps y acumular

    for (int i=0; i<VECTOR_SIZE; i+=AVX_FLOATS){
        __m256 a_vec=simd_loadu_ps(&a[i]);
        __m256 b_vec=simd_loadu_ps(&b[i]);
        __m256 prod=simd_mul_ps(a_vec, b_vec);
        result += simd_reduce_add_ps(prod);
    }
    //&a[0] -> puntero que apunta al primer elemento
    // &a[8] -> puntero que apunta al noveno elmento de la matriz
    return result;
}

...
__global__ void softmax_rows_kernel(const float *input, float *output, int rows,
    ...
    for (int col = tid; col < cols; col += blockDim.x) {
        // Actualizar local_max con el maximo de la fila.
	    local_max = fmaxf(local_max, input[rows * cols + col]);
    }
    ...
    for (int stride = blockDim.x / 2; stride > 0; stride >>= 1) {
        if (tid < stride) {
            //Reducir los maximos usando fmaxf.
    ...
    for (int col = tid; col < cols; col += blockDim.x) {
        ...
        // Calcular expf(input[idx] - row_max), guardelo en output[idx]
    ...
    for (int col = tid; col < cols; col += blockDim.x) {
        int idx = row * cols + col;
        //Normalisar output[idx] dividiendo entre row_sum.
